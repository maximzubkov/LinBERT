...
def eval(
    ...
    mem_params = sum([param.nelement() * param.element_size() for param in model.bert.parameters()])
    mem_bufs = sum([buf.nelement() * buf.element_size() for buf in model.bert.buffers()])

    return end - start, mem_params + mem_bufs
