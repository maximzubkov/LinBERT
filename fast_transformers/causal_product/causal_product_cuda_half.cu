...
typedef torch::PackedTensorAccessor<half, 4, torch::RestrictPtrTraits> half_accessor;
...
__device__ void get_result(
    ...
    for (int l=0; l<L; l++) {
        ...
        __syncthreads();
        half res = queries[n][h][l][e]*kv[n][h][e][m];
        atomicAdd(
            &result[n][h][l][m],
            res
        );
...
__global__ void causal_dot_product_kernel(
    ...
    half* shared_kv = shared_mem;
    half* shared_results = shared_mem + shared_kv_size;
    half* shared_values = shared_results + M;
    half* shared_keys = shared_values + M*T;
    half* shared_queries = shared_keys + E_per_block*T;
    ...
    for (int t=0; t<t_end; t++) {
        ...
        __syncthreads();
        half res = shared_queries[t*E_per_block + e_local] * shared_kv[e_local*M + m];
        atomicAdd(
            &shared_results[m],
            res
        );
        __syncthreads();
        if (threadIdx.x < M) {
            half r1 = shared_results[threadIdx.x];
            atomicAdd(
                &result[n][h][l][m],
                r1
            );
...
void causal_dot_product(
    ...
    const int shared_mem_forward = ((T*shared_mem_per_time) + shared_mem_const) * sizeof(half);

    for (int l_offset=0; l_offset < L; l_offset += T) {
     causal_dot_product_kernel
            <<<blocks, MUL_PER_BLOCK, shared_mem_forward>>>(
            queries.packed_accessor<half, 4, torch::RestrictPtrTraits>(),
            keys.packed_accessor<half, 4, torch::RestrictPtrTraits>(),
            values.packed_accessor<half, 4, torch::RestrictPtrTraits>(),
            kv.packed_accessor<half, 4, torch::RestrictPtrTraits>(),
            product.packed_accessor<half, 4, torch::RestrictPtrTraits>(),
...
__global__ void causal_dot_backward_query_key_kernel(
    ...
    const half_accessor keys,
    const half_accessor values,
    const half_accessor grad_out,
    half_accessor kv,
    half_accessor kv_backwards,
    half_accessor grad_queries,
    half_accessor grad_keys,
    int N,
    int H,
    int L,
    int E,
    ...
    half* shared_kv = shared_mem;
    half* shared_kv_bw = shared_mem + shared_kv_size;
    half* shared_results = shared_kv_bw + shared_kv_size;
    half* shared_results_bw = shared_results + E;
    half* shared_keys = shared_results_bw + E;
    half* shared_values = shared_keys + E*T;
    half* shared_gradout = shared_values + M_per_block*T;
    half* shared_queries_bw = shared_gradout + M_per_block*T;
    half* shared_values_bw = shared_queries_bw + E*T;
    half* shared_gradout_bw = shared_values_bw + M_per_block*T;
    ...
    for (int t=0; t<t_end; t++) {
        ...
        __syncthreads();
        half res = shared_gradout[t*M_per_block + m_local] * shared_kv[m_local*E + e];
        half res_bw = shared_values_bw[t*M_per_block + m_local] * shared_kv_bw[m_local*E + e];
        atomicAdd(
            &shared_results[e],
            res
        );
        ...
        __syncthreads();
        if (threadIdx.x < E) {
            half rq = shared_results[threadIdx.x];
            half rk = shared_results_bw[threadIdx.x];
            atomicAdd(
...
__global__ void causal_dot_backward_value_kernel(
    ...
    half* shared_kv = shared_mem;
    half* shared_results = shared_mem + shared_kv_size;
    half* shared_gradout = shared_results + M;
    half* shared_keys = shared_gradout + M*T;
    half* shared_queries = shared_keys + E_per_block*T;
    ...
    for (int t=0; t<t_end; t++) {
        ...
        __syncthreads();
        half res = shared_keys[t*E_per_block + e_local] * shared_kv[e_local*M + m];
        atomicAdd(
            &shared_results[m],
            res
        );
...
void causal_dot_backward(
    ...
    const int shared_mem_qk_backward = ((T*shared_mem_per_time) + shared_mem_const) * sizeof(half);
    for (int l_offset=0; l_offset < L; l_offset += T) {
        causal_dot_backward_query_key_kernel
            ...
            keys.packed_accessor<half, 4, torch::RestrictPtrTraits>(),
            values.packed_accessor<half, 4, torch::RestrictPtrTraits>(),
            grad_out.packed_accessor<half, 4, torch::RestrictPtrTraits>(),
            kv.packed_accessor<half, 4, torch::RestrictPtrTraits>(),
            kv_backward.packed_accessor<half, 4, torch::RestrictPtrTraits>(),
            grad_queries.packed_accessor<half, 4, torch::RestrictPtrTraits>(),
            grad_keys.packed_accessor<half, 4, torch::RestrictPtrTraits>(),
    ...
    const int shared_mem_v_backward = ((T*shared_mem_per_time) + shared_mem_const) * sizeof(half);
    kv.zero_();
    for (int l_offset=0; l_offset < L; l_offset += T) {
        causal_dot_backward_value_kernel
            ...
            kv.packed_accessor<half, 4, torch::RestrictPtrTraits>(),
            grad_keys.packed_accessor<half, 4, torch::RestrictPtrTraits>(),
            grad_values.packed_accessor<half, 4, torch::RestrictPtrTraits>(),
